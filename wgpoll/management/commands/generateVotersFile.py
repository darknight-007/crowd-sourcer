...
class Command(BaseCommand):
	...
	def handle(self, *args, **options):
		...
		myfile=open('ballotsOceans2013.csv','w')
		angle = {'unsure' : -1 , 'east' : 0, 'west': 180, 'stop': -10, 'north':90, 'south':270}		
		for state in wgStates:
			myfile.write(str(calendar.timegm(state.time.utctimetuple())) + ',' + str(state.unsure)  + ',' + str(state.stop)  + ',' + str(state.north)  + ','  + str(state.south)  + ','  + str(state.east)  + ',' + str(state.west)  + ','  + str(angle[state.winner]) + '\n')
		print wgStates[0]
